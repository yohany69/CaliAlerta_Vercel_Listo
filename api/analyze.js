...
- arbol → árbol caído, rama caída, árbol inclinado, poda urgente → DAGMA: 313 780 7532
- incendio → fuego, humo, llamas → Bomberos Cali: 602 882 1252
- electrico → poste dañado, transformador, cableado eléctrico roto → EMCALI Energía: 602 899 9999
- cable → cable caído al piso, cable tirando chispa, cable de luz suelto → EMCALI Energía: 602 899 9999
- semaforo → semáforo dañado, apagado, mal → Secretaría de Movilidad Cali: 602 369 0811
- via → hueco en carretera, vía dañada, bache, pavimento roto → Secretaría de Infraestructura: 602 885 1325
- inundacion → calle inundada, agua acumulada, desbordamiento → Bomberos Cali: 602 882 1252
- puente → puente dañado, grietas en puente, puente en riesgo → Secretaría de Infraestructura: 602 885 1325
- acueducto → fuga de agua potable, tubería rota, alcantarilla rota → EMCALI Acueducto: 602 899 9999
- gas → olor a gas, fuga de gas, tubería de gas → Gases de Occidente: 6024187333
- construccion → construcción ilegal peligrosa, andamio mal puesto → Planeación Municipal: 602 885 1325
- choque → accidente de tránsito, carros chocados → Policía de Tránsito: 602 369 0811
- basura → acumulación enorme de basura, escombros ilegales → EMSIRVA: 602 552 2010
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!response.ok) {
      return res.status(response.status).json({ error: data?.error?.message || 'Error al consultar Anthropic.' });
    }
